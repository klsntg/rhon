  ...
  return (
    <div>
      ...
      <section>
        ...
        <p className='total-estimated-value'>${totalValue.toLocaleString()}</p>
      </section>
      <section>
        <h2>Assets</h2>
          ...
          {cryptoAssets.filter(asset => asset.amount > 0).map((asset, index) => (
            <div key={index}>
              ...
              <div className='asset-card'>
                ...
                <p className='asset-amount'>{asset.amount.toLocaleString('en-US', {maximumFractionDigits: 8 })} </p>
                <p className='asset-price'>(${(asset.amount * asset.current_price).toFixed(2).toLocaleString()})</p>
              </div>
            </div>
          ))}
        </div>
